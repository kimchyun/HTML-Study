	 ...
	 */
	 ...
	 $(document).on('click', '.action', function(){
		 ...
		 if(vname == "delete"){
			 ...
			 //저장하기 위해서는 renum, bonum(vidx), cont(vtval), name이 필요하다.
			 ...
			 if($('#modiform').css('display') != "none"){
				 //열려있다
				 replyReset();  //이미 열려 있는 modiform을 body 안으로 이동시키는 역할
			 }
...
// 			 $('#modiform').css('display', "block");
	 ...
	 //modiform - 댓글 수정해서 취소 버튼 클릭 이벤트
	 $('#modireset').on('click', function(){
		 replyReset();
	 
     })
     
	 //modiform - 댓글 수정해서 확인 버튼 클릭 이벤트
     $('#modisend').on('click', function(){
    	 ...
    	 //<br>로 변경한 새로 입력한 내용을 p3에 출력 - db 수정 성공 후에 하기
